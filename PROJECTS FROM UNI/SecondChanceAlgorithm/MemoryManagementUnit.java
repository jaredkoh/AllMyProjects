...
public class MemoryManagementUnit {
    ...
    public MemoryManagementUnit(int pageTableSize, int bitsForOffset) {
        ...
        for (int i = 0; i < (pageTableSize / 2); ++i) {
            freeFrames.addLast((1 << bitsForOffset) * i);
        }
    ...
    private void pageFaultHandler(int pageNumber) {
    	...
    	if(freeFrames.isEmpty() == false && pageNumber < freeFrames.size()){
    		pageTable[pageNumber] = new PageTableEntry(freeFrames.getFirst());
    		al.add(pageNumber);
    		...
    		for(int i = 0 ; i < al.size(); i++){
    			int check = al.get(i);
    			  if(pageTable[check].getReferenceBit()){
    	         		pageTable[check].setReferenceBit(false);
    	         	}
    	    	  else{
    	    		  
    	             	pageTable[check].setReferenceBit(true);
    	             	frameAddress = pageTable[i].getAddress();
    	         		pageTable[check].setValidBit(false);
    	         		PageTableEntry pageTableEntry = new PageTableEntry(frameAddress);
    	         		pageTable[check] = pageTableEntry;
    	         		al.set(i, check);

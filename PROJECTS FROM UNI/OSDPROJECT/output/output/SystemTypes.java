...
public interface SystemTypes
{
  ...
  public static boolean exists_0(List _l,PatientGP patientgpx)
  { 
    for (int _i = 0; _i < _l.size(); _i++)
    { PatientH q = (PatientH) _l.get(_i);
      if (q.getforename().equals(patientgpx.getname1()) && q.getsurname().equals(patientgpx.getname2()) && q.getagemnths() == ( 20141200 - patientgpx.getdob() ) / 100) { return true; }
    }
    return false;
  }
  ...
  { List _results_2 = new Vector();
    for (int _i = 0; _i < _l.size(); _i++)
    { Referral referral_2_xx = (Referral) _l.get(_i);
     _results_2.add(new Integer(referral_2_xx.getdate()));
    }
    return _results_2;
  }
